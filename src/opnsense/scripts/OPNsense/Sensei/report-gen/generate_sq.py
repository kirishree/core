...
def set_time(query, chart):
    ...
    query = query.replace('__INTERVAL__', str(INTERVAL))
    query = query.replace('__SIZE__', '10')
    return query


def set_criteria(query, index, type):
    if index == 'conn' and CRITERIA in ['volume', 'packets'] and type not in ['custom']:
        query = query.replace('count(*)', 'sum(dst_nbytes)' if CRITERIA == 'volume' else 'sum(dst_npackets)')
       
    if index == 'http' and CRITERIA == 'volume':
        query = query.replace('count(*)', 'sum(rsp_body_len)')
    
    return query.replace('__WHERE__', '')

def execute_query(dbpath,query, index):
    total = 0
    result = []
    try:
        sqfile = dbpath + '/' + (index if index != 'threat' else 'conn') + '_all.sqlite'
        logging.info('db: ' + sqfile )
        if os.path.exists(sqfile):
            ...
            doc = None
            for doc in rows:
                if 'total' in dict(doc):
                    total += dict(doc)['total']
                if doc is not None:        
                    result.append(dict(doc))
    except Exception as e:
        logging.error(f'SQlite Query Error: {repr(e)}')
        pass        
    return result,total
...
def generate_sq(dbpath,PDF,logging):
    ...
    for r in reports_config:
        logging.info('SQlite preparing report %s' % r['name'])
        if not r['enabled']:
            continue

        graphData = json.loads('{}')
        logging.info('SQlite Generated: %s => %s => %s' % (r['index'], r['name'],r['type']))
        filepath = os.path.join(BASE_DIR, 'queries_sq/%s/%s.sql' % (r['index'], r['name']))
        if not os.path.exists(filepath):
            logging.error(f"SQlite Report Name: {r['name']}, {filepath} not found.")
            continue    
        ...
        with open(filepath, 'r') as file:
            ...
            data,total = execute_query(dbpath, query, r['index'])
            
            # print(data,total)

        if len(data) == 0:
            logging.info(f"SQlite: Zero data for graph {r['name']}")
            continue
        index = 0
        graphData['name'] = r['name']
        if r['name'] != 'conn_facts':
            for row in data:
                if type(row['label']) is int:
                    data[index]['label'] = '%d' % row['label']
                index += 1
        if r['name'] == 'conn_facts' and len(data) > 0:
            ...
            conn_facts.append(['Unique Local Hosts', '{:,}'.format(int(data['e']))])
            conn_facts.append(['Unique Remote Hosts', '{:,}'.format(int(data['f']))])
            conn_facts.append(['Unique Apps', '{:,}'.format(int(data['g']))])
        ...
        if r['name'] == 'conn_table_apps':
            ...
            for i,b in enumerate(data):
                conn_table_apps.append([b['app_name'], b['total'],
                    b['ip_src_saddr'],b['ip_dst_saddr'],
        ...
        if r['name'] == 'conn_table_local_assets':
            ...
            for i,b in enumerate(data):
                key = f"{b['src_hostname']} ({b['ip_src_saddr']})" if  b['ip_src_saddr'] != b['src_hostname'] else b['src_hostname']
                conn_table_apps.append([key, b['total'],
                    b['src_hostname'],b['app_name'],
                    byte_format(b['total_src_nbytes']),
                    byte_format(b['total_dst_nbytes']),
                    b['total_src_npackets'],b['total_dst_npackets']])
        ...
        if r['name'] == 'conn_table_remote_hosts':
            ...
            table_headers = ['Remote Hosts','Sessions','Unique Remote Hosts','Unique Apps'
                ,'Bytes OUT','Bytes IN','Pkts OUT','Pkts IN']
            for i,b in enumerate(data):
                key = f"{b['dst_hostname']} ({b['ip_dst_saddr']})" if  b['ip_dst_saddr'] != b['dst_hostname'] else b['dst_hostname']
                conn_table_apps.append([key, b['total'],
                    b['dst_hostname'],b['app_name'],
                    byte_format(b['total_src_nbytes']),
                    byte_format(b['total_dst_nbytes']),
                    b['total_src_npackets'],b['total_dst_npackets']])
        ...
        if r['type'] == 'doughnut':
            ...
            sum = 0
            for row in data:
                label = row['label'] if row['label'] != '' else 'anonymous' if 'src_username' in query or 'dst_username' in query else 'blank'
        ...
        if r['type'] == 'line-1':
            ...
            for row in data:
                if row['label'] not in labels:
                    ...
                    for h in X_AXIS:
                        tmp[h] = 0        
                    label_val[row['label']] = tmp;    
                    labels.append(row['label'])
                time_str = datetime.utcfromtimestamp(row['_interval'] * 36000) + TZ
                time_str = time_str.strftime('%H:00')
                label_val[row['label']][time_str] = row['total']
        ...
        if r['type'] == 'stacked':
            ...
            for row in data:
                if row['label'] not in label_x:
                    label_x.append(row['label'])
                if row['labels'] not in label_y:
                    label_y.append(row['labels'])
